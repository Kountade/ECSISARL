      ...
      <div className="bg-base-100 rounded-xl shadow-md border border-base-200 p-3">
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <div className={`${showFilters ? 'flex' : 'hidden'} sm:flex gap-2 flex-wrap`}>
            <select className="select select-bordered select-sm w-36" value={filterCustomer} onChange={(e) => { setFilterCustomer(e.target.value); setCurrentPage(1) }}>
            ...
            <select className="select select-bordered select-sm w-32" value={filterPaymentStatus} onChange={(e) => { setFilterPaymentStatus(e.target.value); setCurrentPage(1) }}>
              <option value="">Statut paiement</option>
              {Object.entries(paymentStatusConfig).map(([key, config]) => (<option key={key} value={key}>{config.label}</option>))}
            </select>
            <button className="btn btn-outline btn-sm gap-1" onClick={() => { setFilterCustomer(''); setFilterStatus(''); setFilterPaymentStatus(''); setSearchTerm(''); setCurrentPage(1) }}>
              ...
              <table className="table table-xs w-full">
                <thead>
                  <tr className="text-xs">
                    <th><button className="flex items-center gap-1 hover:text-primary" onClick={() => handleSort('sale_number')}>Vente<ArrowUpDown className="w-3 h-3" /></button></th>
                    ...
                    <th><button className="flex items-center gap-1 hover:text-primary" onClick={() => handleSort('sale_date')}>Date<ArrowUpDown className="w-3 h-3" /></button></th>
                    <th>Livraison</th>
                  ...
                  {paginatedSales.map((sale) => {
                    ...
                    return (
                      <tr key={sale.id} className="hover">
                        <td>
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-base-content/50">{sale.items?.length || 0} produit(s)</p>
                            </div>
                          </div>
                        </td>
                        <td><div className="flex items-center gap-1"><Users className="w-3 h-3 text-primary" /><span className="text-sm">{sale.customer_name}</span></div></td>
                        <td className="text-sm">{new Date(sale.sale_date).toLocaleDateString('fr-FR')}</td>
                        <td className="text-sm">{sale.delivery_date ? new Date(sale.delivery_date).toLocaleDateString('fr-FR') : '-'}</td>
                        ...
                        <td className="text-center"><span className={`badge badge-${paymentStatus.color} badge-sm`}>{paymentStatus.label}</span></td>
                        <td className="text-right font-bold text-sm">{formatNumber(sale.total)} €</td>
                        <td className="text-center">
                          <div className="flex items-center justify-center gap-1">
                            <button onClick={() => navigate(`/ventes/${sale.id}`)} className="btn btn-ghost btn-xs" title="Détails"><Eye className="w-3.5 h-3.5" /></button>
                            {sale.status === 'draft' && (
                              <>
                                <button onClick={() => navigate(`/ventes/${sale.id}/modifier`)} className="btn btn-ghost btn-xs text-primary" title="Modifier"><Edit className="w-3.5 h-3.5" /></button>
                                <button onClick={() => handleConfirmSale(sale.id)} className="btn btn-ghost btn-xs text-success" title="Confirmer"><CheckCircle className="w-3.5 h-3.5" /></button>
                                <button onClick={() => { setSaleToDelete(sale); setShowDeleteModal(true) }} className="btn btn-ghost btn-xs text-error" title="Supprimer"><Trash2 className="w-3.5 h-3.5" /></button>
                              </>
                            )}
                            {sale.status === 'confirmed' && (
                              <button onClick={() => handleCancelSale(sale.id)} className="btn btn-ghost btn-xs text-error" title="Annuler"><XCircle className="w-3.5 h-3.5" /></button>
                            )}
                          </div>
                        </td>
                      </tr>
                ...
                <tfoot className="bg-base-100 border-t-2">
                  <tr className="text-xs font-bold">
                    ...
                    <td className="text-right">{formatNumber(sortedSales.reduce((s, sale) => s + (sale.total || 0), 0))} €</td>
                    <td></td>
                  </tr>
                </tfoot>
              </table>
        ...
        <div className="modal modal-open">
          <div className="modal-box w-11/12 max-w-md p-4">
            <div className="text-center mb-3">
              ...
              <p className="text-xs text-base-content/70">Supprimer la vente <strong>"{saleToDelete.sale_number}"</strong> ?</p>
            ...
            <div className="flex gap-2">
              ...
              <button className="btn btn-sm btn-error flex-1" onClick={handleDeleteSale}>Supprimer</button>
            </div>
          </div>
        </div>
      )}
